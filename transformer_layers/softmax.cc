...
void Softmax::compute(quant_bit_width *input, std::size_t seq_len){
    ...
    for (int i =0; i< seq_len; i++){
        ...
        quant_bit_width max_val = input[i* seq_len];
        ...
        for (int j = 0; j < width; j++) {
            input[i * seq_len + j] = (quant_bit_width) std::max(input[i * seq_len + j] - max_val, -32767);
        }
        ...
        for (int j = 0; j < width; j++) {
            ...
            input[i*seq_len + j] = (quant_bit_width) (input_float *  (1 << NUM_FRACTION_BITS));
            sum += input[i*seq_len + j];
        }

        float sum_float = (float) sum / (float) (1 << NUM_FRACTION_BITS);
        auto sum_inv = (float) (1/(sum_float + 0.00001)); // prevent zero divide!
        auto sum_inv_int = (quant_bit_width) (sum_inv * (1 << NUM_FRACTION_BITS));
        // Normalize the row so that the sum of all elements is 1.
        for (int j = 0; j < width; j++) {
            input[i*seq_len + j] = (quant_bit_width) MUL(input[i*seq_len +j], sum_inv_int);
        }
    }
}
