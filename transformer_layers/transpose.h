#pragma once

#include "util.h"
#include "../transformer.h"

class Transpose {
public:
    static void transpose(const uint32_t* input, uint32_t* output, std::size_t width, std::size_t height) ;
    static void transpose(const quant_bit_width * input, quant_bit_width* output, std::size_t width, std::size_t height) ;
    static void transpose_rearranged(uint32_t* input, uint32_t* output, std::size_t width, std::size_t height,
                                     std::size_t, std::size_t) ;

    static void multihead_transpose(const quant_bit_width* input, quant_bit_width* output, std::size_t seq_len,
                                        std::size_t head_hidden_size, std::size_t num_head);
};